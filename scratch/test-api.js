const BASE_URL = process.argv.find(arg => arg.startsWith('--url='))?.split('=')[1] || 'http://localhost:3001';
...
async function test() {
  ...
  try {
    ...
    if (!res.ok || !body.success) {
      console.error('Failed to log in as admin. Please ensure you ran the promote-user.js script and the server is running.');
    ...
    console.error('Please ensure the Next.js dev server is running.');
    process.exit(1);
  }
  ...
  // 8. Test search functionality
  {
    ...
    if (res.ok && Array.isArray(body.data)) {
      const found = body.data.some(u => (u._id || u.id) === createdUserId);
      if (found) {
