...
import { 
  ArrowLeft, Heart, Bell, Shield, LogOut, User, 
  Upload, CheckCircle, Sparkles, Building, Info 
} from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { saveLocalProperty, Property } from '../../lib/propertiesData';
...
export default function AddPropertyPage() {
  ...
  const validate = () => {
    ...
    if (type !== 'Land' && (!beds || Number(beds) <= 0)) newErrors.beds = 'Required';
    if (type !== 'Land' && (!baths || Number(baths) <= 0)) newErrors.baths = 'Required';
    if (!sqft || Number(sqft) <= 0) newErrors.sqft = 'Please enter valid area size';
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const generatedId = `prop-local-${Date.now()}`;
    ...
    const newProp: Property = {
      ...
      priceFormatted: `NPR ${formattedPrice.toLocaleString()}`,
      ...
      sqft: Number(sqft),
      badges: [
        { text: type === 'Apartment' ? 'For Sale' : 'Just Listed', type: isHotListing ? 'hot' : 'sale' },
        ...(isAiVerified ? [{ text: 'AI Verified' as const, type: 'ai' as const }] : []),
        ...(isValuePick ? [{ text: 'Value Pick' as const, type: 'value' as const }] : [])
      ],
      ...
      description,
      landlord: {
        id: `landlord-${user.email}`,
        name: user.name,
        phone: '+977 98510-99999',
        email: user.email,
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', background: '#f8fafc', minHeight: '100vh', color: '#1e293b' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '32px 16px' }}>
        ...
        {/* Back Link */}
        <Link href="/dashboard" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', textDecoration: 'none', color: '#64748b', fontWeight: 600, fontSize: '0.95rem', marginBottom: '24px' }}>
          <ArrowLeft size={18} /> Back to Search Feed
        </Link>

        {/* Heading Header */}
        <div style={{ marginBottom: '32px' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, color: '#0f172a', margin: '0 0 8px 0' }}>Post New Property</h1>
          <p style={{ color: '#64748b', fontSize: '1rem', margin: 0 }}>List your home, villa, or apartment directly onto Gharpurja search feeds.</p>
        </div>

        {/* Form Card */}
        <form onSubmit={handleSubmit} style={{ background: '#ffffff', borderRadius: '20px', border: '1px solid #e2e8f0', padding: '32px', display: 'flex', flexDirection: 'column', gap: '24px', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
          
          {/* Section 1: Core Details */}
          <div>
            <h3 style={{ fontSize: '1.15rem', fontWeight: 700, borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '16px', color: '#0f172a' }}>1. Basic Information</h3>
            ...
            <div className="form-grid-2col">
              <div className="form-group">
                ...
                />
                {errors.sqft && <div className="form-error-msg">{errors.sqft}</div>}
              </div>
            ...
            {type !== 'Land' && (
              <div className="form-grid-2col">
                <div className="form-group">
                  ...
                  />
                  {errors.beds && <div className="form-error-msg">{errors.beds}</div>}
                </div>
                ...
                <div className="form-group">
                  ...
                  />
                  {errors.baths && <div className="form-error-msg">{errors.baths}</div>}
                </div>
              </div>
            )}
          </div>
          ...
          <div>
            <h3 style={{ fontSize: '1.15rem', fontWeight: 700, borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '16px', color: '#0f172a' }}>2. Property Media</h3>
            
            <div className="form-group">
              <label className="form-label">Choose Preset High-Quality Image *</label>
              <div className="preset-images-grid">
                {PRESET_IMAGES.map((img, i) => (
                  <div 
                    key={i} 
                    onClick={() => { setSelectedImage(img.path); setCustomImage(''); }}
          ...
          <div>
            <h3 style={{ fontSize: '1.15rem', fontWeight: 700, borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '16px', color: '#0f172a' }}>3. Details & Amenities</h3>
            
            <div className="form-group">
              <label className="form-label" style={{ marginBottom: '12px' }}>Select Amenities</label>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
                ...
                <label className="checkbox-label" style={{ fontWeight: 600 }}>
                  <input type="checkbox" checked={waterBackup} onChange={(e) => setWaterBackup(e.target.checked)} />
                  Water Backup Supply
                </label>
              </div>
            </div>
            ...
            <div className="form-group" style={{ marginTop: '16px' }}>
              <label className="form-label" style={{ marginBottom: '12px' }}>Gharpurja AI Insights & Badges</label>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
                <label className="checkbox-label" style={{ fontWeight: 600 }}>
                  <input type="checkbox" checked={isAiVerified} onChange={(e) => setIsAiVerified(e.target.checked)} />
                  Request AI Verification
                </label>
                <label className="checkbox-label" style={{ fontWeight: 600 }}>
                  <input type="checkbox" checked={isValuePick} onChange={(e) => setIsValuePick(e.target.checked)} />
                  Mark as Value Pick
                </label>
                <label className="checkbox-label" style={{ fontWeight: 600 }}>
                  <input type="checkbox" checked={isHotListing} onChange={(e) => setIsHotListing(e.target.checked)} />
                  Mark as Hot Listing
                </label>
              </div>
            </div>
          ...
          <div>
            <h3 style={{ fontSize: '1.15rem', fontWeight: 700, borderBottom: '1px solid #e2e8f0', paddingBottom: '8px', marginBottom: '16px', color: '#0f172a' }}>4. Property Description</h3>
            ...
            <div className="form-group">
              ...
              <textarea 
                ...
                style={{ width: '100%', padding: '12px', border: '1px solid #e2e8f0', borderRadius: '10px', fontSize: '0.9rem', outline: 'none', background: '#f8fafc', resize: 'vertical' }}
            ...
            {/* Landlord Alert Note */}
            <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '12px', padding: '16px', display: 'flex', gap: '12px', alignItems: 'flex-start', marginTop: '12px' }}>
              <Info size={20} style={{ color: '#16a34a', flexShrink: 0, marginTop: '2px' }} />
              <div style={{ fontSize: '0.85rem', color: '#15803d', lineHeight: 1.5 }}>
                <strong>Landlord listing details:</strong> By posting this property, your profile (<strong>{user.name}</strong>, {user.email}) will be tagged as the primary landlord of contact. Ensure your settings are updated to receive customer texts.
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            style={{ margin: '12px 0 0 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
          >
      ...
      {showSuccessModal && (
        <div className="modal-overlay">
          <div className="modal-container" style={{ textAlign: 'center', padding: '32px' }}>
            ...
            <h3 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '8px', color: '#0f172a' }}>Property Posted Successfully!</h3>
            <p style={{ color: '#64748b', fontSize: '0.95rem', lineHeight: 1.5, marginBottom: '24px' }}>
              Your property has been successfully listed in the Gharpurja local database. It is now searchable on all tenant dashboards.
            </p>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <Link href={`/properties/${newPropertyId}`} style={{ display: 'block', background: '#4f46e5', color: '#ffffff', padding: '12px', borderRadius: '10px', fontWeight: 700, textDecoration: 'none', transition: 'background 0.2s' }} className="btn-modal-action">
                View Posted Property Detail
              </Link>
              <Link href="/dashboard" style={{ display: 'block', border: '1px solid #cbd5e1', color: '#475569', padding: '12px', borderRadius: '10px', fontWeight: 700, textDecoration: 'none', transition: 'background 0.2s' }} className="btn-modal-action-secondary">
                Go to Search Feed
              </Link>
            </div>
          </div>
